...
const TeacherExamDutyList = () => {
  ...
  const upcomingCount = exams.filter((e) => String(e.date || '').slice(0, 10) >= today).length;

  const groups = useMemo(() => {
    const list = exams.filter((e) => {
      const d = String(e.date || '').slice(0, 10);
      return tab === 'upcoming' ? d >= today : d < today;
    });
    const map = new Map();
    list.forEach((e) => {
    ...
    return [...map.values()]
      .map((g) => ({
        ...g,
        rows: g.rows.sort((a, b) => String(a.date).localeCompare(String(b.date)) || String(a.time).localeCompare(String(b.time))),
      }))
      ...
      <div className="mx-auto max-w-5xl space-y-5">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            ...
            <div className="rounded-xl bg-slate-50 p-3"><p className="text-2xl font-semibold text-slate-900">{exams.length - upcomingCount}</p><p className="mt-1 text-xs text-slate-500">Completed</p></div>
          </div>
        </section>
          ...
          groups.map((g) => (
            <section key={g.title} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <header className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-4 py-3 sm:px-5">
                ...
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${g.published ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                  {g.published ? 'Routine published' : 'Provisional'}
                </span>
                    ...
                    {g.rows.map((e) => {
                      ...
                      return (
                        <tr key={e._id} className="border-t border-slate-100">
                          ...
                          <td className="px-4 py-2.5 text-slate-700">{e.subjectId?.name || e.subject || '—'}</td>
                          <td className="px-4 py-2.5 text-slate-700">{[e.classId?.name || e.grade, e.sectionId?.name || e.section].filter(Boolean).join(' – ') || '—'}</td>
